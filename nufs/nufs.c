             ...
             off_t offset, struct fuse_file_info *fi)
{
    ...
    for (slist* xs = items; xs != 0; xs = xs->next) {
        ...
        printf("IN: %s\n", path);
        // item_path[0] = '/';
        // memcpy(item_path + 1, xs->data, 127);
        // printf("SEARCHING FOR IT IN: %s\n", item_path);
        ...
        else
        {
            ...
            memcpy(item_path + strlen(path) + 1, xs->data, 128-1-strlen(path));
        }
...
{
    int rv = nufs_mknod(path, 16877, 0);
    printf("mkdir(%s) -> %d\n", path, rv);
    return rv;
}
...
{
    printf("MOVE FROM %s TO %s", from, to);
